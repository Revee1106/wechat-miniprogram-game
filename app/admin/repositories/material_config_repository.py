...
class MaterialConfigRepository:
    def __init__(self, base_path: Path | str | None = None) -> None:
        ...
        self._config_dir = root / "config" / "materials"
        self._items_path = self._config_dir / "items.json"

    def load(self) -> dict[str, list[dict[str, Any]]]:
        self._ensure_file_exists()
        return {"items": self._read_json(self._items_path)}

    def save(self, payload: dict[str, list[dict[str, Any]]]) -> None:
        self._config_dir.mkdir(parents=True, exist_ok=True)
        self._write_json(self._items_path, payload.get("items", []))
